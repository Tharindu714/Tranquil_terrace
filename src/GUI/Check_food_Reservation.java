...
public class Check_food_Reservation extends javax.swing.JFrame {
    ...
    public Check_food_Reservation() {
        ...
        loadRes("SELECT *FROM `kot`\n"
                ...
                + "INNER JOIN `kot_status` ON `kot`.`kot_status_id` = `kot`.`id` "
                + "INNER JOIN `meal_time` ON `kot`.`meal_time_id` = `meal_time`.`id`"
                + "INNER JOIN `food_category` ON `food_item`.`id` = `food_category`.`id`"
                + "INNER JOIN (SELECT `kot_id`, `food_item_id`, `qty`FROM `kot_has_food`) AS `food_qty`\n"
                + "ON `kot_has_food`.`kot_id` = `food_qty`.`kot_id`\n"
                + "AND `kot_has_food`.`food_item_id` = `food_qty`.`food_item_id`");
    }
