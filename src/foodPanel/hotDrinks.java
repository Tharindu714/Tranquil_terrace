...
public class hotDrinks extends javax.swing.JPanel {
    ...
    private void tea() {
        try {
            ResultSet rs = MySQL.execute(commonQuery + " WHERE`id` ='38'");
            if (rs.next()) {
    ...
    private void coffee() {
        try {
            ResultSet rs = MySQL.execute(commonQuery + " WHERE`id` ='39'");
            if (rs.next()) {
                String foodname = rs.getString("name");
                String foodprice = rs.getString("price");
    ...
    private void hot_chocolate() {
        try {
            ResultSet rs = MySQL.execute(commonQuery + " WHERE`id` ='40'");
            if (rs.next()) {
                String foodname = rs.getString("name");
                String foodprice = rs.getString("price");
    ...
    private void jImagePanel1MouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_jImagePanel1MouseClicked
        ...
        try {
            ...
            if (resultset.next()) {
                ...
                JOptionPane.showMessageDialog(this, "Tea Added", "SUCCESSFUL", JOptionPane.INFORMATION_MESSAGE);
            }
        } catch (Exception e) {
            Dashboard.log.warning(e.toString());
        }
    ...
    private void jImagePanel2MouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_jImagePanel2MouseClicked
        ...
        try {
            ...
            if (resultset.next()) {
                ...
                JOptionPane.showMessageDialog(this, "Coffee Added", "SUCCESSFUL", JOptionPane.INFORMATION_MESSAGE);
            }
        } catch (Exception e) {
            Dashboard.log.warning(e.toString());
        }
       ...
       String DateTime = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(new Date());
        try {
            ...
            if (resultset.next()) {
                ...
                JOptionPane.showMessageDialog(this, "Hot Chocolate Added", "SUCCESSFUL", JOptionPane.INFORMATION_MESSAGE);
            }
        } catch (Exception e) {
            Dashboard.log.warning(e.toString());
        }
